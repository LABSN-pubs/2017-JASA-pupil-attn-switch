...
makeweb: bib/switching.bib manuscript.tex pandoc/latex-postprocessor.py linkpdf
	python pandoc/latex-postprocessor.py manuscript.tex \
		McCloyEtAl-pupil-switching-manuscript.tex
...
makesub: bib/switching.bib submission.tex pandoc/latex-postprocessor.py \
		pandoc/latex-make-standalone.py linkeps
	...
	python ./pandoc/latex-make-standalone.py submission-temp.tex \
		McCloyEtAl-pupil-switching.tex
...
makepre: bib/switching.bib prepress.tex pandoc/latex-postprocessor.py linkpdf
	python pandoc/latex-postprocessor.py prepress.tex \
		McCloyEtAl-pupil-switching-prepress.tex
...
manuscript.tex: manuscript/manuscript.md bib/manuscript-numeric.bst \
		pandoc/template-JASA-manuscript.tex makefigs
	ln -sf bib/manuscript-numeric.bst pupil-switching.bst
	pandoc --filter pandoc-eqnos --natbib --no-tex-ligatures \
		--template=pandoc/template-JASA-manuscript.tex --output=manuscript.tex \
		manuscript.md

submission.tex: manuscript.md bib/jasa-submission.bst \
		pandoc/template-JASA-submission.tex figures/fig-1.eps figures/fig-2.eps \
		figures/fig-3.eps
	ln -sf bib/jasa-submission.bst pupil-switching.bst
	pandoc --filter pandoc-eqnos --natbib \
		--template=pandoc/template-JASA-submission.tex --output=submission.tex \
		manuscript.md

prepress.tex: manuscript.md bib/jasa-submission.bst \
		pandoc/template-JASA-prepress.tex makefigs
	ln -sf bib/jasa-submission.bst pupil-switching.bst
	pandoc --filter pandoc-eqnos --natbib \
		--template=pandoc/template-JASA-prepress.tex --output=prepress.tex \
		manuscript.md

bib/manuscript-numeric.bst: bib/manuscript-numeric.dbj bib/bst-patcher.py
	cd bib; tex manuscript-numeric.dbj
	cd bib; python bst-patcher.py manuscript-numeric-unpatched.bst \
		manuscript-numeric.bst 
...
bib/jasa-submission.bst: bib/jasa-submission.dbj bib/bst-patcher.py
	...
	cd bib; python bst-patcher.py jasa-submission-unpatched.bst \
		jasa-submission.bst 
...
cleanweb: cleancommon
	...
	bn="McCloyEtAl-pupil-switching-manuscript"; for ext in $(TEXEXTS); \
		do rm -f "$$bn.$$ext"; done

cleansub: cleancommon
	rm -f *.eps *-eps-converted-to.pdf submission.tex submission-temp.tex \
		submission-temp.pdf
	bn="submission-temp"; for ext in $(TEXEXTS); do rm -f "$$bn.$$ext"; done
	bn="McCloyEtAl-pupil-switching"; for ext in $(TEXEXTS); \
		do rm -f "$$bn.$$ext"; done

cleanpre: cleancommon
	rm -f fig-*.pdf *prepress.tex
	bn="McCloyEtAl-pupil-switching-prepress"; for ext in $(TEXEXTS); \
		do rm -f "$$bn.$$ext"; done
...
clean: cleanweb cleansub cleanpre
	rm -f McCloyEtAl-pupil-switching.pdf $(EPSFIGS) $(PDFFIGS) \
		McCloyEtAl-pupil-switching-manuscript.pdf
