...
for subj in subjects:
    ...
    for kernel in kernels:
        ...
        # reorder fits to match big_mat (stim# in serial order, rev/anec, time)
        order = np.array(run_inds)
        revb_idx = (order[:, :, 1] - 1).ravel()
        ...
        fits_ordered[stim_idx, revb_idx, :] = fit
        zscores_ordered[stim_idx, revb_idx, :] = zscore
        assert not np.any(fits_ordered == np.inf)
        assert not np.any(zscores_ordered == np.inf)

        # now break up by condition (trial, mm/mf, attn, revb/anech, time)
        n_attn = len(np.unique(big_mat[:, 0]))
        n_gend = len(np.unique(big_mat[:, 1]))
        n_revb = len(np.unique(np.array(run_inds)[:, :, 1]))
        trials_per_cond = len(big_mat) / n_attn / n_gend
        fits_structured = np.empty((trials_per_cond, n_gend, n_attn, n_revb,
                                    len(deconv_time_pts)))
        zscores_structured = np.empty((trials_per_cond, n_gend, n_attn, n_revb,
                                       epochs.n_times))
        for ai in range(n_attn):
            for gi in range(n_gend):
        ...
        if run_continuous_deconv:
            ...
            for _trial in range(zscores_zeropadded.shape[0]):
                for _gend in range(zscores_zeropadded.shape[1]):
                    for _attn in range(zscores_zeropadded.shape[2]):
                        for _revb in range(zscores_zeropadded.shape[3]):
                            signal = zscores_zeropadded[_trial, _gend, _attn,
                                                        _revb, :]
                            (fit_continuous_deconv[_trial, _gend, _attn,
                                                   _revb, :],
...
np.savez_compressed(op.join(work_dir, 'rev_data.npz'), **out_nopress)
